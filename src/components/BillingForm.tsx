...
const BillingForm = ({subscriptionPlan} : BillingFormProps) => {
  ...
  const {mutate : createStripeSession , isSuccess , isPending} = trpc.createStripeSession.useMutation({
    onSuccess : ({url})=>{
        ...
        <form className='mt-12' onSubmit={(e)=>{
            ...
            <Card>
                ...
                <CardFooter className='flex flex-col items-start space-y-2 md:flex-row md:justify-between md:space-x-0'>
                    <Button type='submit'>{isPending ? (<Loader2 className='mr-4 h-4 w-4 animate-spin'/>) : isSuccess ? null : null } 
