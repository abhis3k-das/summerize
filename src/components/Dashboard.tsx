...
const Dashboard = () => {
  ...
  return (
    <main className='mx-auto max-w-7xl md:p-10'>
      <div className='mt-10 flex flex-col items-start justify-between gap-4 border-b border-gray-200 pd-5 sm:flex-row sm:items-center sm:gap-0'>
        <h1 className='mb-3 font-bold text-5xl text-gray-900'>My Files</h1>
        <UploadButton/>
      </div>
      {files && files?.length !==0 ? (

        <ul className='mt-10 grid grid-cols-1 gap-6 divide-y divide-zinc-200 md:grid-cols-2 lg:grid-cols-3'>

          {files.sort((a,b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()).map((file)=>(
